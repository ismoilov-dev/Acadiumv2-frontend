...
export default function ChatInterface() {
  ...
  useEffect(() => {
    ...
    if (savedStr) {
      try {
        ...
        if (
          saved.lesson_id &&
          (saved.generation_status === "processing" ||
            saved.generation_status === "pending")
        ) {
  ...
  useEffect(() => {
    ...
    if (isActive) {
      interval = setInterval(() => {
        setProgress((prev) => {
          const newProgress =
            prev >= 90 ? 90 : prev + Math.floor(Math.random() * 5) + 1;
          const currentStep =
            newProgress < 30
              ? "Analyzing prompt"
              : newProgress < 60
  ...
  const handleRenameLesson = async (e) => {
    ...
    try {
      ...
      setHistory((prev) =>
        prev.map((l) =>
          l.id === actionLesson.id ? { ...l, title: renameTitle } : l,
        ),
      );
    ...
    <div className="flex h-[100dvh] w-full bg-slate-50 overflow-hidden font-sans text-slate-900">
      ...
      >
        <div className="flex h-14 items-center justify-between px-4 border-b border-slate-200 bg-slate-50">
          ...
          <button
            onClick={() => setIsSidebarOpen(false)}
            className="lg:hidden p-1 text-slate-500 hover:text-slate-700 hover:bg-slate-200 rounded-md"
          >
            <svg
              className="w-6 h-6"
              fill="none"
            ...
            {!id ? (
              <div className="flex flex-col items-center justify-center pt-20 text-center px-4">
                ...
                <h2 className="text-2xl font-bold text-slate-900 mb-2">
                  Qanday dars o'tmoqchisiz?
                </h2>
                <p className="text-slate-500 mb-8 max-w-md">
                  Mavzuni erkin tilda yozing. AI o'zi mos fan, sinf va rejalarni
                  tuzib beradi.
                </p>
            ...
            lesson?.status === "pending" ? (
              ...
              <div className="flex flex-col gap-3">
                <form
                  ...
                  className="relative flex items-end gap-2 bg-white rounded-2xl border border-slate-300 shadow-sm focus-within:border-indigo-500 focus-within:ring-1 focus-within:ring-indigo-500 transition-all p-1.5 sm:p-2"
                >
                  <textarea
                    value={prompt}
                ...
                <p className="text-center text-[10px] sm:text-xs text-slate-400 mt-1">
                  AI xato qilishi mumkin. O'quvchilarga taqdim etishdan oldin
                  tekshirib chiqing.
                </p>
              </div>
            ) : null}
      ...
      {contextMenu.visible && (
        ...
        >
          <button
            ...
            onClick={() => {
              setActionLesson({
                id: contextMenu.lessonId,
                title: contextMenu.lessonTitle,
              });
